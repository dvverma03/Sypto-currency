...
import { addUser } from "../store/userSlice";
  ...
  export default function OtpVerificationLogin(props) {
    const e1 = useRef("");
    const e2 = useRef("");
    const e3 = useRef("");
    const e4 = useRef("");
    ...
    const {email, password}  = props.route.params?.userData
  ...
  const handleVerifyRegister = async () => {
    try {
      ...
      console.log("use", UserOtp)
      console.log("use", email)
      const res = await axios.post(
        'https://sypto-trading-backend.vercel.app/verify-otp-login',
        {
          email:email, UserOtp:UserOtp
        },
      );
      dispatch(addUser(res?.data))
      navigation.navigate("Home",{"name":res?.data.fullName});
    } catch (error) {
